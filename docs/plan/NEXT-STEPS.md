...
- **Bros tab, Phase A (2026-08-15)** — Community feed + follower model cut (PRD §5.6/§5.8). Third tab is now the **Bros tab** (the leaderboard/challenges screen embedded as a tab); Home's leaderboard card switches tabs instead of pushing a duplicate. Feed UI (`lib/features/community/`) deleted; dormant Supabase `posts`/`post_likes`/`post_comments` + `community-images` await a cleanup migration. **Phase B next:** friendships table + requests + @username + invite link/QR + block/report. **Phase C:** challenges backend (duels/squad/open, rate-limited nudges).
- **Per-lift strength ranks (2026-08-06)** — the 5 barbell classics (bench/squat/deadlift/OHP/row) get a Bronze→Elite rank from all-time e1RM ÷ bodyweight via hand-authored per-sex standards (`lib/features/exercises/lift_rank/`, fully local, no schema change). Surfaced as a frosted rank card on the exercise-detail Summary tab, a full-screen rank-up overlay mid-workout (replaces the PR banner when a band is crossed), and a "Ranks" share-card template (session-best e1RM). Reuses `Rank`/`RankBadge`. Tests in `test/lift_rank_test.dart`. Follow-ups deliberately deferred: profile ranks strip, rep-based ranks for bodyweight moves, gender editor in Settings.
